...
//const int32_t START_TESTIMAGES = 3682, END_TESTIMAGES = 7481;
//const int32_t START_TESTIMAGES = 3799, END_TESTIMAGES = 3800;
const int32_t START_TESTIMAGES = 0, END_TESTIMAGES = 3799;
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  //std::cout << "\t DetSize: " << det.size() << " " << std::flush;
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty) {
  ...
  for (int32_t i=START_TESTIMAGES; i<END_TESTIMAGES; i++){
    ...
    //std::cout << "[ " << i << " / " << END_TESTIMAGES << " ] " << std::flush;
    //std::cout << "len(gt) = " << groundtruth[i].size() << std::endl << std::flush;
    //std::cout << "len(det) = " << detections[i].size() << std::endl << std::flush;
...
bool eval(string result_sha,Mail* mail){
  ...
  string gt_dir         = "/SSD/datasets/KITTI/evaluation/data/object/label_2";
  string result_dir     = ".";
  ...
  for (int32_t i=START_TESTIMAGES; i<END_TESTIMAGES; i++) {
    ...
    // file name
    char file_name[256];
    sprintf(file_name,"%06d.txt",i + 3682);
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("ERROR: Couldn't read: %s of ground truth. Please write me an email!", (gt_dir + "/" + file_name).c_str() );
      return false;
    }
    if (!det_success) {
